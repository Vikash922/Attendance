...
object FirestoreSyncService {
    ...
    /**
     * Resolves the current user's root Firestore document (by UID, falling back to lowercase
     * email), shared by every sync path so they all agree on where a user's data lives.
     */
    private fun resolveUserDoc(db: FirebaseFirestore, profile: UserProfile): com.google.firebase.firestore.DocumentReference? {
    ...
    /**
     * Builds the aggregate metadata map (totals, summaries) written to the root user document.
     * Shared by both the full sync (syncDataToCloud) and the lightweight metadata-only sync
     * (syncMetadataOnly) so the two never drift apart.
     */
    private fun buildMetadataMap(
        ...
        val currentMonthStr = com.example.core.util.LaborCalendarHelper.formatYearMonth(com.example.core.util.LaborCalendarHelper.todayYear, com.example.core.util.LaborCalendarHelper.todayMonth)
        val currentDateTimeStr = java.text.SimpleDateFormat("MMM dd, yyyy hh:mm a", java.util.Locale.US).format(java.util.Date())
        ...
        try {
            FirebaseAuth.getInstance().currentUser?.metadata?.creationTimestamp?.let {
                creationDate = java.text.SimpleDateFormat("MMM dd, yyyy", java.util.Locale.US).format(java.util.Date(it))
            }
        } catch (e: Exception) {}
    ...
    /**
     * FULL sync — re-uploads the root metadata document, the profile, and EVERY worker and
     * transaction document. This is intentionally "expensive" and must only be used by
     * safety-net call sites that need guaranteed full cloud consistency:
     *   - backupAndLogout() / the explicit "Backup Now" action
     *   - The first sync right after a fresh login/signup
     *   - Legacy-data auto-migration
     * Routine "user just tapped something" auto-sync should use syncMetadataOnly() instead,
     * since individual worker/transaction documents are already saved incrementally via
     * saveWorker()/saveTransaction() at the point of each edit.
     */
    suspend fun syncDataToCloud(
        ...
        return try {
            val uid = FirebaseAuth.getInstance().currentUser?.uid
            val userEmail = (if (!profile.email.isNullOrBlank()) profile.email else FirebaseAuth.getInstance().currentUser?.email ?: "").trim().lowercase()
            val userDoc = resolveUserDoc(db, profile)
                ?: return Result.failure(Exception("No user logged in and no email provided"))

            val metadata = buildMetadataMap(profile, workers, transactions, uid, userEmail)
    ...
    /**
     * INCREMENTAL sync — updates ONLY the root metadata document (aggregate totals/summary),
     * without touching any individual worker or transaction document. Safe to call frequently
     * (e.g. on every debounced auto-save) because it does not re-upload any worker/transaction
     * data — those are already kept in sync individually via saveWorker()/saveTransaction()
     * at the moment each one is edited. This is the routine "user just tapped something" path.
     */
    suspend fun syncMetadataOnly(
        profile: UserProfile,
        workers: List<LaborWorker>,
        transactions: List<CashTransaction>,
        context: Context? = null
        ...
        return try {
            ...
            Result.success("Metadata synced (${workers.size} workers, ${transactions.size} transactions).")
        } catch (e: Exception) {
            Log.d(TAG, "Metadata sync note: ${e.message}")
            Result.failure(e)
        }
    }
    ...
    suspend fun downloadDataFromCloud(context: Context? = null, fallbackEmail: String? = null): Result<BackupData> {
        ...
        return try {
            ...
            val backup = BackupData(
                ...
                backupTimestamp = java.text.SimpleDateFormat("dd MMM, hh:mm a", java.util.Locale.US).format(java.util.Date()),
            ...
            if (rawAttendance != null) {
                for ((key, value) in rawAttendance) {
                    ...
                    if (value is Map<*, *>) {
                        ...
                        val rawOtAmount = (value["overtimeAmount"] as? Number)?.toDouble()
                        val otAmount = if (rawOtAmount != null && rawOtAmount > 0.0) {
                            rawOtAmount
                        } else if (otHours > 0.0 && otRate > 0.0) {
                            otHours * otRate
                        } else {
                            0.0
                        }
