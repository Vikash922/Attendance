...
data class DailyAttendance(
    ...
    val overtimeAmount: Double = 0.0, // Hard-locked explicit amount to prevent corruption
...
data class LaborWorker(
    ...
    fun calculateMonthStats(monthStr: String = LaborCalendarHelper.formatYearMonth(LaborCalendarHelper.todayYear, LaborCalendarHelper.todayMonth)): WorkerMonthStats {
        ...
        for (rec in monthAtt.values) {
            ...
            val recordOtAmount = if (rec.overtimeAmount > 0.0) {
                rec.overtimeAmount
            } else {
                (rec.overtimeHours * rec.overtimeRate)
            }
            totalOtAmount += recordOtAmount
        }
