...
object PdfReportGenerator {
    ...
    private fun formatTxDateForPdf(tx: CashTransaction): String {
        ...
        if (tx.dateDisplay.isNotBlank()) {
            ...
            if (parts.isNotEmpty() && parts[0].all { it.isDigit() }) {
                val (y, m) = if (tx.fullDate.isNotBlank()) {
                    LaborCalendarHelper.parseYearMonth(tx.fullDate)
                } else if (tx.timestamp > 0) {
                    val cal = Calendar.getInstance().apply { timeInMillis = tx.timestamp }
                    Pair(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH) + 1)
                } else {
                    Pair(LaborCalendarHelper.todayYear, LaborCalendarHelper.todayMonth)
                }
                val monthYearStr = LaborCalendarHelper.formatYearMonth(y, m)
    ...
    fun shareCashBookReportPdf(context: Context, transactions: List<CashTransaction>, startDate: String, endDate: String, isWhatsApp: Boolean = false) {
        ...
        val timestamp = SimpleDateFormat("yyyyMMdd_HHmm", Locale.US).format(Date())
        saveAndSharePdf(context, document, "Cash_Book_Ledger_$timestamp.pdf", isWhatsApp)
    }
    ...
    private fun saveAndSharePdf(context: Context, document: PdfDocument, fileName: String, isWhatsApp: Boolean = false) {
        try {
