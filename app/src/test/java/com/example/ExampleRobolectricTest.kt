...
class ExampleRobolectricTest {
  ...
  fun `test worker repository add and update salaryType persistence`() = runBlocking {
    ...
    // Verify adding OT hours without custom rate leaves overtimeRate at 0.0 (does not auto-calculate from wage)
    repo.updateDayDetails(
      workerId = worker.id,
      monthStr = "2026-08",
      dayNumber = 11,
      advance = 0.0,
    ...
    val workerWithDefaultOt = repo.workers.value.find { it.id == worker.id }
    assertNotNull(workerWithDefaultOt)
    val defaultOtDay = workerWithDefaultOt!!.attendance["2026-08-11"]
    assertNotNull(defaultOtDay)
    assertEquals(0.0, defaultOtDay!!.overtimeRate, 0.01)

    // Verify custom OT rate (e.g. 100.0) is strictly preserved
    repo.updateDayDetails(
      workerId = worker.id,
      monthStr = "2026-08",
      dayNumber = 10,
    ...
    val workerWithOt = repo.workers.value.find { it.id == worker.id }
    assertNotNull(workerWithOt)
    val attendanceDay = workerWithOt!!.attendance["2026-08-10"]
    assertNotNull(attendanceDay)
    assertEquals(AttendanceStatus.UNMARKED, attendanceDay!!.status)
    assertEquals(3.0, attendanceDay.overtimeHours, 0.01)
    assertEquals(100.0, attendanceDay.overtimeRate, 0.01)
  }
